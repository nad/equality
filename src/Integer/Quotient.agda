...
+-assoc = elim _ λ m₁ n₁ → elim _ λ _ _ → elim _ λ _ _ →
  cong [_] $ cong₂ _,_
    (Nat.+-assoc m₁)
    (Nat.+-assoc n₁)

-- Data._+_ is associative.

Data-+-assoc : ∀ i j k → i Data.+ (j Data.+ k) ≡ (i Data.+ j) Data.+ k
Data-+-assoc i j k = _↔_.injective (inverse ℤ↔ℤ)
  (_↔_.from ℤ↔ℤ (i Data.+ (j Data.+ k))                ≡⟨ sym $ +≡+ i ⟩
   _↔_.from ℤ↔ℤ i + _↔_.from ℤ↔ℤ (j Data.+ k)          ≡⟨ cong (λ j → _↔_.from ℤ↔ℤ i + j) $ sym $ +≡+ j ⟩
   _↔_.from ℤ↔ℤ i + (_↔_.from ℤ↔ℤ j + _↔_.from ℤ↔ℤ k)  ≡⟨ +-assoc (_↔_.from ℤ↔ℤ i) _ _ ⟩
   (_↔_.from ℤ↔ℤ i + _↔_.from ℤ↔ℤ j) + _↔_.from ℤ↔ℤ k  ≡⟨ cong (_+ _↔_.from ℤ↔ℤ k) $ +≡+ i ⟩
   _↔_.from ℤ↔ℤ (i Data.+ j) + _↔_.from ℤ↔ℤ k          ≡⟨ +≡+ (i Data.+ j) ⟩∎
   _↔_.from ℤ↔ℤ ((i Data.+ j) Data.+ k)                ∎)

private
  module Data-ℤ-group = Data.ℤ-group Data-+-assoc

-- The group of integers (from Data).

Data-ℤ-group : Group lzero
Data-ℤ-group = Data-ℤ-group.ℤ-group
...
ℤ-group .Group.left-identity  = elim _ λ _ _ → refl _
ℤ-group .Group.right-identity = elim _ λ _ _ →
  cong [_] $ cong₂ _,_
        Nat.+-right-identity
        Nat.+-right-identity
ℤ-group .Group.left-inverse = elim _ λ _ n →
  []-respects-relation (cong (_⊕ 0) $ Nat.+-comm n)
ℤ-group .Group.right-inverse = elim _ λ m _ →
  []-respects-relation (cong (_⊕ 0) $ Nat.+-comm m)

-- The two groups of integers are isomorphic.
...
-- The two groups of integers are equal (assuming univalence).
...
ℤ×ℤ-not-cyclic =
  ...
  Cyclic (Data-ℤ-group G.× Data-ℤ-group)  ↝⟨ Data-ℤ-group.ℤ×ℤ-not-cyclic ⟩□
  ⊥                                       □
...
ℤ≄ᴳℤ×ℤ =
  ...
  Data-ℤ-group ≃ᴳ (Data-ℤ-group G.× Data-ℤ-group)  ↝⟨ Data-ℤ-group.ℤ≄ᴳℤ×ℤ ⟩□
  ⊥                                                □
