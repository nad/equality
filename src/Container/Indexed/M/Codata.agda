...
import Container.Indexed P.equality-with-J as PC
...
M-final {C = C} =
  ...
  Final′→Final
    ...
    ( M-coalgebra C
    , _≃_.from (Final′≃Final′ (M-coalgebra C)) M-final′
    )
...
H-level-M :
  ...
  H-level n (M C i)
H-level-M {C = C} =
  F.H-level-final-coalgebra ext (M-coalgebra C , M-final)
