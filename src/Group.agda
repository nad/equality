...
module Group {e⁺} (eq : ∀ {a p} → Equality-with-J a p e⁺) where
...
open import Groupoid eq as G using (Groupoid)
...
-- Some of these properties might not be propositional.
...
≃ᴳ→Generated-by→Generated-by
  ...
  let i , x≡ = gen-by (_≃_.from (G₁≃G₂ .related) x) in
    i
