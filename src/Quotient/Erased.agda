...
data _/ᴱ_ (A : Set a) (R : A → A → Set r) : Set (a ⊔ r) where
  [_]                      : A → A /ᴱ R
  @0 []-respects-relationᴾ : R x y → [ x ] P.≡ [ y ]
...
record Elimᴾ′ {A : Set a} {R : A → A → Set r} (P : A /ᴱ R → Set p) :
              Set (a ⊔ r ⊔ p) where
  no-eta-equality
  field
    []ʳ : ∀ x → P [ x ]
...
record Elim {A : Set a} {R : A → A → Set r} (P : A /ᴱ R → Set p) :
            Set (a ⊔ r ⊔ p) where
  no-eta-equality
  field
    []ʳ : ∀ x → P [ x ]
...
record Elim-prop
         {A : Set a} {R : A → A → Set r} (P : A /ᴱ R → Set p) :
         Set (a ⊔ r ⊔ p) where
  no-eta-equality
  field
    []ʳ                : ∀ x → P [ x ]
