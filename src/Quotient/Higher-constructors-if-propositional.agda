...
record Rec-prop {A : Type a} (R : A → A → Type r) (B : Type b) :
                Type (a ⊔ r ⊔ b) where
  no-eta-equality
  field
    []ʳ             : A → B
...
/-map-∥∥ :
  ...
  A₁ / R₁ → A₂ / R₂
/-map-∥∥ {R₁} {R₂} p A₁→A₂ R₁→R₂ = rec λ where
  .[]ʳ → [_] ∘ A₁→A₂

  .is-setʳ → /-is-set ∘ p

  .[]-respects-relationʳ {x} {y} prop →
     R₁ x y                      ↝⟨ ∣_∣ ⟩
     ∥ R₁ x y ∥                  ↝⟨ R₁→R₂ _ _ ⟩
  ...
  { surjection = record
    { logical-equivalence = record
      { to =
          ...
          elim λ where
            .[]ʳ → curry [_]
            .[]-respects-relationʳ {x} {y} prop r → ⟨ext⟩ λ P[y] →
