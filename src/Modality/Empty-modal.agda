...
import Modality.Very-modal eq M as VM
...
------------------------------------------------------------------------
-- Results that are stated under the assumption that the modality is
-- empty-modal

module Empty-modal (Modal-⊥ : Empty-modal M) where
  ...
  -- accompanying "Modalities in Homotopy Type Theory" by Rijke,
  ...
  module Excluded-middle (em : Excluded-middle a) where
    ...
    -- The modality is accessibility-modal for stable relations on
    -- modal types (assuming function extensionality).
    ...
    --------------------------------------------------------------------
    -- Some results that hold if the modality is very modal (in
    -- addition to being empty-modal)
    ...
    module Very-modal (very-modal : Very-modal M) where
      ...
      -- function, and the right-to-left direction of the equivalence
      -- is pointwise equal to the modal unit (assuming function
      -- extensionality).
      ...
      -- The modality is equal to one instance of the identity
      -- modality (assuming function extensionality and univalence).
      ...
      -- The modality is topological (assuming function
      -- extensionality).
...
----------------------------------------------------------------------
-- Some results that are stated using the double-negation monad

-- I had proved some lemmas under the assumption of excluded middle
-- (at the time of writing these lemmas are available above). Felix
-- Cherubini asked whether the lemmas could be proved using
-- double-negation instead, and found a proof for (a variant of) the
-- first lemma below.

-- For a given type it is not not the case that, if the modality is
-- empty-modal and the type is a proposition, then the type is
-- modal.
...
¬¬-Empty-modal→Is-proposition→Modal {A} =
  [ (A                                             →⟨ (λ inhabited prop →
                                                         ...
                                                         propositional⇒inhabited⇒contractible prop inhabited) ⟩
     (Is-proposition A → ↑ _ ⊤ ≃ A)                →⟨ (∀-cong _ λ _ → flip Modal-respects-≃ Modal-⊤) ⟩
     (Is-proposition A → Modal A)                  →⟨ const ⟩□
     (Empty-modal M → Is-proposition A → Modal A)  □)
  , (¬ A                                           →⟨ inverse ∘ _⇔_.from (≃⊥≃¬ _) ⟩
     ⊥ ≃ A                                         →⟨ Modal-respects-≃ ⟩
     (Empty-modal M → Modal A)                     →⟨ flip ∘ const ⟩□
     (Empty-modal M → Is-proposition A → Modal A)  □)
  ] ⟨$⟩ excluded-middle

-- For a given type it is not not the case that, if function
-- extensionality holds and the modality is empty-modal and very
-- modal, then the type is modal.
...
-- extensionality holds and the modality is empty-modal and very
-- modal, then
-- * the modal operator applied to the type is equivalent to the type,
--   and
-- * the right-to-left direction of the equivalence is equal to the
--   modal unit.

¬¬-Empty-modal→Very-modal→◯≃id :
  ¬¬ (Extensionality a a →
      Empty-modal M →
      Very-modal M →
...
¬¬-Empty-modal→Very-modal→◯≃id {A} =                                $⟨ ¬¬-Empty-modal→Very-modal→Modal ⟩
  ...
  ¬¬ (Extensionality a a → Empty-modal M → Very-modal M →
      ∃ λ (eq : ◯ A ≃ A) → _≃_.from eq ≡ η)                         □
