...
private
  ...
  opaque
    ...
    map-cong-correct {ℓ = lower} {x≡y = gx≡gy} f (Cong g x≡y) h =
      ...
      cong (f ∘ g) x≡y     ∎
    map-cong-correct {ℓ = middle} {x≡y} f (No-Sym x≈y) h =
      cong f x≡y           ≡⟨ map-cong-correct f x≈y h ⟩∎
      ⟦ map-cong f x≈y ⟧S  ∎
    map-cong-correct {ℓ = middle} {x≡y} f (Sym y≈x) h =
    ...
    map-cong-correct {ℓ = upper} {x≡y} f Refl h =
      cong f x≡y       ≡⟨ cong (cong f) h ⟩
      cong f (refl _)  ≡⟨ cong-refl f ⟩∎
      refl _           ∎
    map-cong-correct {ℓ = upper} {x≡y} f (Cons x≈y y≈z) h =
      cong f x≡y                                     ≡⟨ cong (cong f) h ⟩
