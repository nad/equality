...
open import Prelude
  hiding (zero; suc; _^_) renaming (_+_ to _⊕_; _*_ to _⊛_)
...
abstract
  ...
  -- false for zero. Leading zeros (at the end of the lists) are not
  -- allowed.
  --
  ...
  private
    ...
    -- Synonyms that can be used in type signatures (Bin is abstract,
    -- so the constructors cannot be used there).
    ...
    -- The invariant implies that the last element in the underlying
    -- list, if any, is not false, so there are no leading zeros in
    -- the number.
  ...
  private
    ...
    -- The successor of the smart cons constructor applied to false
    -- and n is the smart cons constructor applied to true and n.
...
abstract
  ...
  private
    ...
    to-ℕ-add-with-carry₁ :
      ∀ b cs → to-ℕ (add-with-carry₁ b cs) ≡ from-Bool b ⊕ to-ℕ cs
    to-ℕ-add-with-carry₁ false n = refl _
    to-ℕ-add-with-carry₁ true  n = to-ℕ-suc n

    to-ℕ-add-with-carry₂ :
      ∀ b m n →
      to-ℕ (add-with-carry₂ b m n) ≡ from-Bool b ⊕ (to-ℕ m ⊕ to-ℕ n)
    ...
    to-ℕ-add-with-carry₂ false m@(false ∷ m′) n@(false ∷ n′) =
      ...
      2 ⊛ (to-ℕ m′ ⊕ to-ℕ n′)                      ≡⟨ solve 2 (λ c d → con 2 :* (c :+ d) := con 2 :* c :+ con 2 :* d)
                                                        (refl _) (to-ℕ m′) _ ⟩
      2 ⊛ to-ℕ m′ ⊕ 2 ⊛ to-ℕ n′                    ≡⟨⟩
      to-ℕ m ⊕ to-ℕ n                              ∎
    ...
    to-ℕ-add-with-carry₂ false m@(false ∷ m′) n@(true  ∷ n′) =
      ...
      1 ⊕ 2 ⊛ (to-ℕ m′ ⊕ to-ℕ n′)                 ≡⟨ solve 2 (λ c d → con 1 :+ con 2 :* (c :+ d) := con 2 :* c :+ (con 1 :+ con 2 :* d))
                                                       (refl _) (to-ℕ m′) _ ⟩
      2 ⊛ to-ℕ m′ ⊕ (1 ⊕ 2 ⊛ to-ℕ n′)             ≡⟨⟩
      to-ℕ m ⊕ to-ℕ n                             ∎
    ...
    to-ℕ-add-with-carry₂ false m@(true  ∷ m′) n@(false ∷ n′) =
      to-ℕ (true ∷ˢ add-with-carry₂ false m′ n′)  ≡⟨ to-ℕ-∷ˢ true (add-with-carry₂ false m′ n′) ⟩
      1 ⊕ 2 ⊛ to-ℕ (add-with-carry₂ false m′ n′)  ≡⟨ cong ((1 ⊕_) ∘ (2 ⊛_)) (to-ℕ-add-with-carry₂ false m′ n′) ⟩
      1 ⊕ 2 ⊛ (to-ℕ m′ ⊕ to-ℕ n′)                 ≡⟨ solve 2 (λ c d → con 1 :+ con 2 :* (c :+ d) := con 1 :+ con 2 :* c :+ con 2 :* d)
                                                       (refl _) (to-ℕ m′) _ ⟩
      1 ⊕ 2 ⊛ to-ℕ m′ ⊕ 2 ⊛ to-ℕ n′               ≡⟨⟩
      to-ℕ m ⊕ to-ℕ n                             ∎
    ...
    to-ℕ-add-with-carry₂ false m@(true ∷ m′) n@(true ∷ n′) =
      ...
      2 ⊛ (1 ⊕ to-ℕ m′ ⊕ to-ℕ n′)                 ≡⟨ solve 2 (λ c d → con 2 :* (con 1 :+ c :+ d) := con 1 :+ con 2 :* c :+ (con 1 :+ con 2 :* d))
                                                       (refl _) (to-ℕ m′) _ ⟩
    ...
    to-ℕ-add-with-carry₂ true m@(true  ∷ m′) n@(false ∷ n′) =
      to-ℕ (false ∷ˢ add-with-carry₂ true m′ n′)  ≡⟨ to-ℕ-∷ˢ false (add-with-carry₂ true m′ n′) ⟩
      2 ⊛ to-ℕ (add-with-carry₂ true m′ n′)       ≡⟨ cong (2 ⊛_) (to-ℕ-add-with-carry₂ true m′ n′) ⟩
      2 ⊛ (1 ⊕ to-ℕ m′ ⊕ to-ℕ n′)                 ≡⟨ solve 2 (λ c d → con 2 :* (con 1 :+ c :+ d) := con 2 :+ con 2 :* c :+ con 2 :* d)
                                                       (refl _) (to-ℕ m′) _ ⟩
      2 ⊕ 2 ⊛ to-ℕ m′ ⊕ 2 ⊛ to-ℕ n′               ≡⟨⟩
      1 ⊕ to-ℕ m ⊕ to-ℕ n                         ∎
    ...
    to-ℕ-add-with-carry₂ true m@(true ∷ m′) n@(true ∷ n′) =
      ...
      1 ⊕ 2 ⊛ (1 ⊕ to-ℕ m′ ⊕ to-ℕ n′)            ≡⟨ solve 2
                                                      (λ c d → con 1 :+ con 2 :* (con 1 :+ c :+ d) := con 2 :+ con 2 :* c :+ (con 1 :+ con 2 :* d))
                                                      (refl _) (to-ℕ m′) _ ⟩
  ...
  _+_ : Bin → Bin → Bin
  _+_ = add-with-carry₂ false

  to-ℕ-+ : ∀ m n → to-ℕ (m + n) ≡ to-ℕ m ⊕ to-ℕ n
  to-ℕ-+ = to-ℕ-add-with-carry₂ false
  ...
  _*_ : Bin → Bin → Bin
  []      * n = zero
  (b ∷ m) * n = (if b then _+_ n else id) (false ∷ˢ m * n)
  ...
  _^_ : Bin → Bin → Bin
  m ^ []      = suc zero
  m ^ (b ∷ n) = (if b then (m *_) else id) ((m * m) ^ n)
  ...
  to-ℕ-*2^ : ∀ m n → to-ℕ (m *2^ n) ≡ to-ℕ m ⊛ 2 N.^ n
  to-ℕ-*2^ m N.zero =
  ...
  private
    ...
    --
    -- This definition uses Dec-Erased instead of Dec ∘ Erased because
    -- I thought this made the code a little less complicated.
  ...
  to-ℕ-from-bits-to-bits : ∀ n → to-ℕ (from-bits (to-bits n)) ≡ to-ℕ n
