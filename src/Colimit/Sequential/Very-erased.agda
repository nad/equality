...
record Elimᴾ
         ...
         {@0 step₀ : P₀ → P₊ zero}
         {@0 step₊ : ∀ {n} → P₊ n → P₊ (suc n)}
         (Q : Colimitᴱ P₀ P₊ step₀ step₊ → Set q) :
         Set (p₀ ⊔ p₊ ⊔ q) where
  no-eta-equality
  field
    ∣∣₀ʳ        : (x : P₀) → Q ∣ x ∣₀
...
-- Some instances of Colimitᴱ can be converted to instances of
...
-- Colimitᴱ and some instances of Colimit.
...
Colimitᴱ≃Colimit = Eq.↔→≃
  ...
  e′ : C.Elim _
  e′ .C.Elim.∣∣ʳ {n = zero} _     = refl _
  e′ .C.Elim.∣∣ʳ {n = suc _} _    = refl _
  e′ .C.Elim.∣∣≡∣∣ʳ {n = zero} x =
