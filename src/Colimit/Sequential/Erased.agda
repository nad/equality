...
data Colimitᴱ (P : ℕ → Set p) (@0 step : ∀ {n} → P n → P (suc n)) :
             Set p where
...
record Elimᴾ
         {P : ℕ → Set p} {@0 step : ∀ {n} → P n → P (suc n)}
         (Q : Colimitᴱ P step → Set q) : Set (p ⊔ q) where
  no-eta-equality
  field
    ∣∣ʳ       : (x : P n) → Q ∣ x ∣
    @0 ∣∣≡∣∣ʳ :
...
elimᴾ :
  {@0 step : ∀ {n} → P n → P (suc n)} {Q : Colimitᴱ P step → Set q} →
