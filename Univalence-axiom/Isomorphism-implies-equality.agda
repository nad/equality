...
module Another-example where
  ...
  -- A corresponding notion of morphism.
  ...
  isomorphic-equal univ₁ univ₂
    ...
    lemma₁′ =
      _⇔_.to propositional⇔irrelevant
        (H-level-propositional
           (dependent-extensionality univ₁ (λ _ → univ₂)) 2)
        _ _
    ...
    to-lemma : ∀ x → subst (λ A → A) A₁≡A₂ x ≡ to x
    to-lemma x =
      sym $ subst-unique
        (λ A → A)
    ...
    cast-lemma : ∀ f → subst (λ A → A → A) A₁≡A₂ f ≡ cast f
    cast-lemma f =
      sym $ subst-unique
        (λ A → A → A)
